...
(function () {
    ...
    let m28nApiUrl //之所以用 let 类型是因为 url 一直在变
    const nativeWebSocket = unsafeWindow.WebSocket
    unsafeWindow.WebSocket = function (...args) {
        const webSocket = new nativeWebSocket(...args)
        m28nApiUrl = webSocket.url
        return webSocket
    }


    const cp6 = unsafeWindow.cp6
    ...
    function getCentraliaServerInfo() {
    ...
    function getSpecifiedMapServerInfo(mapNumber) {
        fetch(`https://api.n.m28.io/endpoint/florrio-map-${mapNumber}-green/findEach/`)
            .then((response) => response.json())
            .then((data) => {
                let info = {}
                info[`${data.servers["vultr-miami"].id}`] = { "serverArea": "NA", "mapNumber": mapNumber }
                info[`${data.servers["vultr-frankfurt"].id}`] = { "serverArea": "EU", "mapNumber": mapNumber }
                info[`${data.servers["vultr-tokyo"].id}`] = { "serverArea": "AS", "mapNumber": mapNumber }
                Object.assign(serversInfoTemp, info)
                seversInfoReadyCount++
                if (seversInfoReadyCount == serversCount) {
                    serversInfo = serversInfoTemp
                }
            })
    }
    ...
    let serversInfo = {}
    let serversInfoTemp = {}
    const serversCount = 14
    let seversInfoReadyCount = 0
    for (let i = serversCount; i; i--) { getSpecifiedMapServerInfo(i - 1) }


    let positionStyle = undefined
    function getUserServerId() {
        if (typeof (positionStyle) != "undefined") { clearTimeout(positionStyle) }
        const userServerId = m28nApiUrl.slice(6, 9)
        const userServerInfo = serversInfo[userServerId]
        doc.style.top = "0px"
        doc.innerHTML = `服务器连接正常<br>服务器地域: ${userServerInfo["serverArea"]}&nbsp&nbsp服务器ID: ${userServerId}&nbsp&nbsp地图编号: ${userServerInfo["mapNumber"]}<br>${m28nApiUrl.slice(6, -1)}`
        positionStyle = setTimeout(() => { doc.style.top = "-150px" }, 3000)
    }


    function changeServerByRealRegion() {
        const switchServerName = prompt("请输入您需要进入的服务器所在的地域: NA/EU/AS\nPlease enter the region where the server you need to enter: NA/EU/AS")
        if (switchServerName) { cp6.forceServerID(getCentraliaServerInfo[switchServerName.toUpperCase()]["serverId"]) }
    }


    function changeServerById() {
        const switchServerName = prompt("请输入您需要进入的服务器的ID\nPlease enter the ID of the server you need to enter")
        if (switchServerName) { cp6.forceServerID(switchServerName) }
    }
    ...
    httpRequest.open('GET', 'https://fanyi.youdao.com/translate?doctype=json&type=type&i=Hi', true); //第二步：打开连接  将请求参数写在url中  ps:"./Ptest.php?name=test&nameone=testone"
        ...
        setTimeout(() => {
            if (text == "AFK Check") {
                ...
                Notification.requestPermission(function (status) {
                    ...
                    var n = new Notification('AFK！', {
                        body: '您有一个 AFK Check 需要点击！',
    ...
    function getTranslate(text) {
        /*
    脚本DDos轰炸器，1秒1万发访问量，秒杀低配的网站（

            var httpRequest = new XMLHttpRequest();//第一步：建立所需的对象
        httpRequest.open('GET', 'https://florr.ga:2147', true);//第二步：打开连接  将请求参数写在url中  ps:"./Ptest.php?name=test&nameone=testone"
        httpRequest.send();//第三步：发送请求  将请求参数写在URL中
        httpRequest.onreadystatechange = function () {
            if (httpRequest.readyState == 4 && httpRequest.status == 200) {
                var json = httpRequest.responseText;//获取到json字符串，还需解析
    ...
    const compatibleCanvas = getCompatibleCanvas()


    //初始化
    for (const { prototype } of compatibleCanvas) {
    ...
    //接管全局键盘事件
    const onkeydownFunction = window.onkeydown ? window.onkeydown : function (e) { } //储存旧代码
    window.onkeydown = (e) => {
