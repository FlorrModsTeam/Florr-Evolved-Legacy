...
(function () {
    ...
    let cp6 = undefined
    const cp6Loop = setInterval(() => {
        if (typeof (unsafeWindow.cp6) == undefined) { return }
        clearInterval(cp6Loop)
        cp6 = unsafeWindow.cp6
    }, 500)
    ...
    const toast = new class {
        constructor() {
            this.positionStyle = undefined
        }
        popUp(str) {
            if (typeof (this.positionStyle) != "undefined") { clearTimeout(this.positionStyle) }
            doc.innerHTML = str
            doc.style.top = "0px"
            this.positionStyle = setTimeout(() => { doc.style.top = "-150px" }, 3000)
        }
    }
    ...
    let centraliaServerInfo = {}
    getCentraliaServerInfo()
    ...
    function getUserServerId() {
        if (typeof (m28nApiUrl) == "undefined") {
            toast.popUp("请先连接至 Florr.io 的服务器")
            return
        }
        const userServerId = m28nApiUrl.slice(6, 9)
        const userServerInfo = serversInfo[userServerId]
        if (typeof (userServerInfo) == "undefined") {
            toast.popUp("正在初始化，请稍后")
            return
        }
        toast.popUp(`服务器连接正常<br>服务器地域: ${userServerInfo["serverRegion"]}&nbsp&nbsp服务器ID: ${userServerId}&nbsp&nbsp地图编号: ${userServerInfo["mapNumber"]}<br>${m28nApiUrl.slice(6, -1)}`)
    }


    function changeServerByRegion() {
        ...
        if (switchServerName) { cp6.forceServerID(centraliaServerInfo[switchServerName.toUpperCase()]["serverId"]) }
    }
    ...
    function getTranslate(text, textColor) {
    ...
    //普通的日志输出，但加入了防止输出过快的措施
    function preventTooFastLog(string) {
        if (performance.timeOrigin + performance.now() - timestamp < 2333) { return }
    ...
    function alertTargetStr(searchStr, targetStr) {
        if (searchStr.search(targetStr) > -1) { alert(searchStr); }
    }
    ...
    for (const { prototype } of compatibleCanvas) {
        ...
        prototype.fillText = function (text, x, y) {
            return this.rewriteFillText(getTranslate(text, this.fillStyle), x, y);
        }
        ...
        prototype.measureText = function (text) {
            const newFontSize = getBlurFontSize(this.font);
            this.font = getApplicableFontStr(newFontSize);
            return this.rewriteMeasureText(getTranslate(text, this.fillStyle));
        }
    }
