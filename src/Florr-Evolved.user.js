...
`
	...
	fetch("https://api.n.m28.io/endpoint/florrio-map-1-green/findEach/")
		.then((response) => response.json())
		.then((data) => {
			servers2 = [data.servers["vultr-miami"].id, data.servers["vultr-frankfurt"].id, data.servers["vultr-tokyo"].id]
		})
    ...
    脚本DDos轰炸器，1秒1万发访问量，秒杀低配的网站（
        ...
        */
		...
		if (typeof(translate[text]) == 'string') {
		...
		if (openCustomReloadAnimation) {
			...
			prototype.arc = function(x, y, radius, startAngle, endAngle, anticlockwise) {
				if (x == 0 && y == -5 && radius == 120 && anticlockwise == false) {
					...
					if (progress >= 9950) {
						this.globalAlpha = (50 - (progress - 9950)) / 50;
					} //最后的降低透明度动画 | 注意 js 小数运算精度问题
